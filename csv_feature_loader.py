#!/usr/bin/env python3
"""
CSV Feature Loader for ML Pipeline
...
class CSVFeatureLoader:
    """Feature loader that uses prepared CSV files"""
    ...
    def verify_data_consistency(self):
        ...
        if text_train is not None and financial_train is not None:
            ...
            if text_labels is not None and financial_labels is not None:
                common_ids = list(text_ids & financial_ids)
                text_labels_common = text_labels[text_train.index.isin(common_ids)]
                financial_labels_common = financial_labels[financial_train.index.isin(common_ids)]
                
                if np.array_equal(text_labels_common, financial_labels_common):
